...
public class SSOController {
    ...
    public String doLogin(String username,
        ...
        try {
            ...
            if (imoocjsonResult.getStatus() != 200) {
                model.addAttribute("errmsg", imoocjsonResult.getMsg());
                return "login";
            }
            ...
            //8.创建用户会话
            redisOperator.set(REDIS_USER_TOKEN + ":" + result.getId(), JSON.toJSONString(usersVO));
            //9.创建用户全局门票 生成ticket门票，全局门票，代表用户的CAS端登陆过
            String userTicket = UUID.randomUUID().toString().trim();

            //用户全局门票需要放入CAS端的cookie中
            this.setCookie(COOKIE_USER_TICKET, userTicket, response);
            //userTicket关联用户id，并且放入到redis中，代表这个用户有门票了，可以在各个景区游玩
            redisOperator.set(REDIS_USER_TICKET + ":" + userTicket, result.getId());
            //10.创建临时票据 回跳到调用端网站，是由CAS端所签发的一个一次性的临时ticket
            tmpTicket = this.createTmpTicket();
            /*
    ...
    private String createTmpTicket(){
        String tmpTicket = null;
        try {
            tmpTicket = UUID.randomUUID().toString().trim();
